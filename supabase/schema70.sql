-- =====================================================================
-- Calista Concept — schema70.sql
-- Run AFTER schema69.sql.
--
-- CRITICAL FIX — deal approve/reject failing with:
--   22P02  invalid input value for enum deal_status: ""
--
-- Root cause: an old, broken definition of public.sync_deal_payout()
-- (from schema3) casts coalesce(new.status,'') straight into the
-- deal_status enum. If that definition is live, EVERY update to deals
-- fails with this error. This script force-replaces the function with
-- the latest correct definition (schema13, text-cast safe) and
-- refreshes its trigger + the PostgREST schema cache.
...
begin
  -- Statuses are compared as TEXT so no '' ever hits the deal_status enum
...
-- Clean any legacy rows with invalid/empty status (same spirit as schema4)
update public.deals set status = 'pending_review' where status::text = '' or status is null;
