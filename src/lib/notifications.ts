...
export function defaultPreferencesFor(role: 'admin' | 'seller' | 'headhunter'): NotificationKey[] {
  if (role === 'admin') {
    return ['admin_deal_new', 'admin_deal_review', 'admin_lead_new', 'admin_inbox', 'admin_meeting', 'admin_payout_reminder', 'user_lead_reminder', 'user_whats_new', 'user_broadcast']
  }
  return ['user_inbox', 'user_deal_approved', 'user_lead_status', 'user_payout', 'user_lead_reminder', 'user_whats_new', 'user_broadcast']
}
