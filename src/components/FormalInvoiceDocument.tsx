...
export function FormalInvoiceDocument({
  ...
  // Document type label at the very top — defaults to "ORIGINAL FOR
  // CUSTOMER" when the document type is Original (the common case).
  const docIndicator =
  ...
  // Title-bar text — "TAX INVOICE" for invoices, "CREDIT NOTE" for
  // credit notes, etc.  Falls back to "INVOICE" for receipts/proforma.
  const titleBar =
    extras.document_type === 'Credit Note' ? 'TAX INVOICE / CREDIT NOTE'
    : extras.document_type === 'Receipt' ? 'RECEIPT'
    : extras.document_type === 'Proforma' ? 'PROFORMA INVOICE'
    : 'TAX INVOICE'

  // QR code (optional) — generated by the public goqr.me API from the
  // payload the admin configured.  300×300 px image displayed at 25mm
  // gives ~300 DPI for print.
  ...
  return createPortal(
    ...
    >
      ...
      {/* ============================================================ */}
      <div style={{ textAlign: 'center', fontSize: '7pt', letterSpacing: '0.18em', color: '#444', textTransform: 'uppercase', padding: '1mm 0 2mm' }}>
        {docIndicator}
      </div>

      {/* ============================================================ */}
      {/* 2. TITLE BAR — full-width light-gray bar                      */}
      {/* ============================================================ */}
      <div style={{ background: '#D3D3D3', color: '#000', fontWeight: 700, fontSize: '9pt', letterSpacing: '0.15em', textAlign: 'center', padding: '1.5mm 0', margin: '0 0 3mm' }}>
        {titleBar}
      </div>

      {/* ============================================================ */}
      {/* 3. INVOICE IDENTIFICATION — left meta + optional QR right      */}
      {/* ============================================================ */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '0.5pt solid #000', paddingBottom: '2.5mm', marginBottom: '3mm' }}>
        <div style={{ flex: 1, fontSize: '7.5pt' }}>
        ...
        {qrUrl && (
          <div style={{ flexShrink: 0, marginLeft: '6mm', textAlign: 'center' }}>
            ...
            <p style={{ margin: '1mm 0 0', fontSize: '6pt', color: '#555' }}>Scan to verify</p>
          </div>
        )}
      </div>
      ...
      {extras.ship_to && hasAnyText(extras.ship_to) && (
        <div style={{ marginBottom: '3mm', borderTop: '0.25pt solid #000', borderBottom: '0.25pt solid #000', padding: '2mm 0' }}>
          ...
          <div style={{ fontSize: '7.5pt', lineHeight: 1.45, display: 'flex', gap: '6mm' }}>
            <div style={{ flex: 1 }}>
            ...
            {extras.ship_to.delivery_method && (
              <div style={{ flex: '0 0 auto', fontSize: '7pt' }}>
                <p style={{ margin: '0 0 1pt', fontSize: '6.5pt', letterSpacing: '0.1em', color: '#555', textTransform: 'uppercase' }}>Delivery method</p>
      ...
      {extras.references && hasAnyText(extras.references) && (
        <div style={{ marginBottom: '3mm', fontSize: '7pt' }}>
          <SectionHeader>ADDITIONAL REFERENCES</SectionHeader>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4mm 8mm', borderTop: '0.25pt solid #000', paddingTop: '1.5mm' }}>
      ...
      {/* 7. MAIN ITEMS TABLE                                           */}
      {/* ============================================================ */}
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '7.5pt', marginTop: '1mm' }}>
        <thead>
          <tr style={{ background: '#D3D3D3', color: '#000', fontSize: '7pt', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
          ...
          {services.length === 0 ? (
            ...
            <tr key={s.id} style={{ pageBreakInside: 'avoid' }}>
              ...
              <td style={{ border: '0.25pt solid #000', padding: '1.5mm' }}>
                ...
                {s.description && <span style={{ display: 'block', color: '#444', fontSize: '7pt', marginTop: '0.5pt' }}>{s.description}</span>}
              </td>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '2.5mm' }}>
        <table style={{ borderCollapse: 'collapse', fontSize: '7.5pt', width: '55%' }}>
          <tbody>
            <tr>
            ...
            <tr style={{ background: '#D3D3D3' }}>
              <td style={{ padding: '1.5mm 3mm', fontWeight: 700, fontSize: '9pt', textAlign: 'right', borderTop: '0.75pt solid #000', borderBottom: '0.75pt solid #000' }}>TOTAL DUE</td>
              <td style={{ padding: '1.5mm 3mm', fontWeight: 700, fontSize: '10pt', textAlign: 'right', fontVariantNumeric: 'tabular-nums', borderTop: '0.75pt solid #000', borderBottom: '0.75pt solid #000' }}>{eurFull(total)}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      {/* Amount in words */}
      <p style={{ margin: '1.5mm 0 0', textAlign: 'right', fontSize: '7pt', color: '#333', fontStyle: 'italic' }}>
      ...
      <div style={{ marginTop: '3mm', display: 'flex', alignItems: 'center', gap: '4mm', borderTop: '0.25pt solid #000', borderBottom: '0.25pt solid #000', padding: '1.5mm 0' }}>
        <div style={{ flex: '0 0 auto' }}>
          <p style={{ margin: '0', fontSize: '6.5pt', letterSpacing: '0.1em', color: '#555', textTransform: 'uppercase', fontWeight: 700 }}>Reference barcode</p>
          <p style={{ margin: '0.5pt 0 0', fontSize: '6pt', color: '#666' }}>Cross-reference for the digital ledger</p>
        </div>
        <div style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '3mm' }}>
          ...
          <span style={{ fontFamily: '"Courier New", Courier, monospace', fontSize: '7pt', letterSpacing: '0.12em' }}>{invoice.number}</span>
        </div>
      </div>
      ...
      {extras.bank && hasAnyText(extras.bank) && (
        <div style={{ marginTop: '3mm' }}>
          ...
          <div style={{ display: 'flex', gap: '6mm', borderTop: '0.25pt solid #000', paddingTop: '1.5mm', fontSize: '7.5pt' }}>
            <div style={{ flex: 1 }}>
              <p style={{ margin: '0 0 1mm', fontSize: '6.5pt', letterSpacing: '0.1em', color: '#555', textTransform: 'uppercase', fontWeight: 700 }}>Bank details</p>
            ...
            <div style={{ flex: 1 }}>
              <p style={{ margin: '0 0 1mm', fontSize: '6.5pt', letterSpacing: '0.1em', color: '#555', textTransform: 'uppercase', fontWeight: 700 }}>Payment details</p>
      ...
      {(extras.legal_notes || freeform) && (
        <div style={{ marginTop: '2.5mm', fontSize: '7pt', color: '#333', lineHeight: 1.45 }}>
          ...
          <p style={{ margin: '0', fontSize: '6.5pt', color: '#666' }}>
      ...
      {extras.secondary_section && extras.secondary_section.content.trim() && (
        <div style={{ marginTop: '4mm', borderTop: '0.5pt solid #000', paddingTop: '2mm' }}>
          <div style={{ background: '#E7E7E7', fontSize: '7.5pt', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '1mm 2mm', marginBottom: '1.5mm' }}>
            {extras.secondary_section.type}
          </div>
          <p style={{ margin: '0', fontSize: '7pt', color: '#333', whiteSpace: 'pre-wrap', lineHeight: 1.5 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <span>{companyName}{companyVat ? ` · VAT ID: ${companyVat}` : ''}{companyId ? ` · Company ID: ${companyId}` : ''}</span>
          <span>Document {invoice.number}</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '0.5pt' }}>
          <span>{companyEmail}{companyPhone ? ` · ${companyPhone}` : ''}{companyWebsite ? ` · ${companyWebsite}` : ''}</span>
          <span>Printed using CCInvoiceEngine v1.0</span>
        </div>
      </div>
    </div>,
    document.body,
...
function MetaRow({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: 'flex', marginBottom: '0.6mm' }}>
      <span style={{ width: '26mm', fontWeight: 700, color: '#333', flexShrink: 0 }}>{label}:</span>
...
function RefField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div style={{ display: 'flex', gap: '1.5mm' }}>
      <span style={{ fontWeight: 700, color: '#444', fontSize: '6.5pt', letterSpacing: '0.04em' }}>{label}:</span>
...
function SignBlock({ role, name, date }: { role: string; name: string; date: string }) {
  return (
    <div style={{ width: '46%' }}>
      <p style={{ margin: '0 0 1pt', fontSize: '6.5pt', letterSpacing: '0.1em', color: '#444', textTransform: 'uppercase', fontWeight: 700 }}>
        {role}
      </p>
      {name && <p style={{ margin: '0 0 6mm', fontSize: '7.5pt', fontWeight: 600 }}>{name}</p>}
      {!name && <p style={{ margin: '0 0 6mm', fontSize: '7pt', color: '#888' }}>__________________</p>}
      <div style={{ borderTop: '0.25pt solid #000', paddingTop: '1mm', fontSize: '7pt', display: 'flex', justifyContent: 'space-between' }}>
        <span>Signature</span>
